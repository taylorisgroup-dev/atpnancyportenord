...
import { LayoutGrid, List, Building2, Briefcase, Download, Search, MapPin, Phone, Mail } from 'lucide-react';
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '24px' }}>
              {displayed.map((c: any, i: number) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.4, delay: (Math.min(i, 11) % 12) * 0.04 }} style={{ padding: '28px', borderRadius: '24px', position: 'relative', overflow: 'hidden', background: 'white', border: '1px solid #e5e7eb', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px', padding: '12px', background: '#f8fafc', borderRadius: '8px' }}>
                    {c.phone && <div style={{ fontSize: '0.75rem', color: 'var(--charcoal-gray)', display: 'flex', alignItems: 'center', gap: '6px' }}><Phone size={12} style={{ color: 'var(--atp-blue)' }}/> {c.phone}</div>}
                    {c.email && <div style={{ fontSize: '0.75rem', color: 'var(--charcoal-gray)', display: 'flex', alignItems: 'center', gap: '6px' }}><Mail size={12} style={{ color: 'var(--atp-blue)' }}/> <a href={`mailto:${c.email}`} style={{ color: 'inherit', textDecoration: 'none' }}>{c.email}</a></div>}
                    {(!c.phone && !c.email) && <div style={{ fontSize: '0.75rem', color: '#94a3b8', fontStyle: 'italic' }}>Contact non renseigné</div>}
                  </div>
                ...
                <motion.div key={i} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: i * 0.03 }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '15px', marginTop: '6px' }}>
                      {c.phone && <div style={{ fontSize: '0.72rem', color: 'var(--charcoal-gray)', display: 'flex', alignItems: 'center', gap: '4px' }}><Phone size={10} style={{ color: 'var(--atp-blue)' }}/> {c.phone}</div>}
                      {c.email && <div style={{ fontSize: '0.72rem', color: 'var(--charcoal-gray)', display: 'flex', alignItems: 'center', gap: '4px' }}><Mail size={10} style={{ color: 'var(--atp-blue)' }}/> <a href={`mailto:${c.email}`} style={{ color: 'inherit', textDecoration: 'none' }}>{c.email}</a></div>}
                    </div>
                  </div>
